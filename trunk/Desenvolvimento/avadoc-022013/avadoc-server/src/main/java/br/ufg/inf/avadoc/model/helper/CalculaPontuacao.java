...
public final class CalculaPontuacao {
	...
	public static class EstagioProbatorio {
		 ...
		 * 
		 * @param notasNP
		 *            lista de notas a ser calculada a média ponderada
		...
		public static double calculaNF(final List<Nota> notasNP) {
			...
			for (final Nota nota : notasNP) {
				total = total + nota.getNota() * nota.getQuantidadeMeses();
			}

			total = total / EstagioProbatorio.TEMPO;
	...
	public static class ProgressaoHorizontal {
		...
		public static double calculaNCAD(final List<Nota> notasNP) {
			...
			total = total / ProgressaoHorizontal.TEMPO;
		 ...
		 *            nota do interstício
		 * @param notaDiscente
		 *            nota média do corpo discente
		 * @return retorna a nota com base na fórmula:
		 *         (NG = NCAD * 0,7 + ND * 0,3)
		 */
		public static double calculaNG(final double ncad, final double notaDiscente) {
			return ncad * ProgressaoHorizontal.PESO_NCAD + notaDiscente * ProgressaoHorizontal.PESO_ND;
		}
	...
	public static Nota calculaN1(final Nota nota, final EnumJornadaTrabalho jorTra) {
		...
		if (nota.getQuantidadeMeses() <= 0 || nota.getNota() < 0) {
			...
			if (jorTra.equals(EnumJornadaTrabalho.jornada20)) {
				n1.setNota(CalculaPontuacao.QUANTIDADE_MESES * nota.getNota() / nota.getQuantidadeMeses()
						/ CalculaPontuacao.JORNADA_20_HORAS);
			} else {
				n1.setNota(CalculaPontuacao.QUANTIDADE_MESES * nota.getNota() / nota.getQuantidadeMeses()
						/ CalculaPontuacao.JORNADA_40_HORAS_OU_DE);
			}

			n1.setNota(CalculaPontuacao.CONSTANTE_AVALIACAO >= n1.getNota() ? n1.getNota()
					: CalculaPontuacao.CONSTANTE_AVALIACAO);
		}
	 ...
	 * 
	 * @param n1
	 *            nota das atividades do docente
	 * @param notaChefia
	 *            nota da diretoria ou chefia de gabinete
	 * @param notaDiscente
	 *            nota do corpo discente
	 * @return retorna a nota parcial do docente referido à um período
	 */
	public static Nota calculaNP(final Nota n1, final double notaChefia, final double notaDiscente) {

		final Nota notaParcial = new Nota();
		...
		if (notaChefia < 0 || notaDiscente < 0) {
			...
			notaParcial.setNota(n1.getNota() * CalculaPontuacao.PESO_N1 + notaChefia * CalculaPontuacao.PESO_N2
					+ notaDiscente * CalculaPontuacao.PESO_N3);
		}
